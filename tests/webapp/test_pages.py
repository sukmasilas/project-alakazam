...
def test_root_redirects_to_inventory(auth_client):
    resp = auth_client.get("/", follow_redirects=False)
    assert resp.status_code == 303
    assert resp.headers["location"] == "/inventory"


def test_inventory_page_renders(auth_client):
...
def test_purchase_entry_page_renders(auth_client):
